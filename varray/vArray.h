...
//					- all elements in [size, capacity) are initialized with T() (CURRENTLY FOR TESTING PURPOSES: #ifdef _TEST_VARRAY_TEST)
...
//					- [const_]iterator [c]begin/[c]end() family defined.
// 					- [const_]iterator [c]endlim() as after the whole storage defined for Testing. 
...
//    ver 0.2		- RandomAccessIterator defined and implemented
...
//                    no difference between <DTy> and <DTy *> except for nullptr checks
...
#include "..\iterators\class_RAI.h"			// Defines RandomAccessIterator class
...
class varray {
	...
	public:
		...
		// const _RefT	operator [] (const size_t ind) const ;
...
varray<T, A>::varray(size_t num, const T& val, size_t cap, A a) : m_base(a, (cap < num ? (cap = num + 8) : cap))
{
												...
												// if (val == T())	cout << "val = T()" ; else  cout << (val) ; 
												// cout << ")/cap(): " << cap ;
	try {
...
constexpr int SpacePerElement { 8 } ;    // ... # of ' '
